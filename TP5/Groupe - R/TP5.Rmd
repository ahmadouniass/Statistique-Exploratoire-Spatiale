---
title: " PRESENTATION DE LA  AU CAMEROUN"
author: "ONANENA AMANA Jeanne De La Flèche & Khadidiatou COULIBALY & Samba DIENG & Tamsir NDONG"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
header-includes:
  ...
  - \usepackage{geometry}
  - \geometry{margin=2.5cm, includeheadfoot, showframe=false}
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhf{}
  - \fancyhead[C]{}
...
setwd("D:/ISEP3/Semestre 1/Statistique spatiale et exploratoire/Statistique-Exploratoire-Spatiale/Devoir_1/Cameroun/cmr_admbnda_inc_20180104_SHP")
...
ggplot() +
  ...
  geom_sf(data = region, fill = NA, color = "black", size = 0.5) +  # Ajouter les contours des regions
...
```
# 8. Nombre d'enfants atteints par admin et enregistrer le résultat en fichier csv
```{r, echo= FALSE}
...
write.csv(pop_arrondissement, file.path(output_folder, "arondissement_cmr.csv"), row.names = FALSE)
...
```{r, echo= FALSE}
sum(pop_arrondissement$taux_malade)
```
